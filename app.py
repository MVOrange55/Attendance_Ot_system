...
# --- 4. ENGINE FUNCTIONS (CACHED) ---
@st.cache_data(show_spinner=False)
def run_hr_engine(df_serialized, holidays, corrections):
    ...
    for eid in df_w[id_c].unique():
        ...
        for d in dates:
            ...
            if not t_in and not t_out:
                ...
                status, a_c = "A", a_c + 1
                res_mi.append({"ID": clean_id, "Name": ename, "Date": d_i, "Status": "Miss Punch", "Detail": "Punch Missing"})
            else:
                ...
                if is_off_day:
                    ...
                    if t_in >= time(13, 30):
                        ...
                        if t_in and t_in > time(9, 35): late_log.append(f"{d_i}({t_in.strftime('%H:%M')})")
                        if work_hrs < 8.5: early_log.append(f"{d_i}({t_out.strftime('%H:%M') if t_out else 'N/A'})")
        ...
        res_o.append(row_o)
        res_ex.append({"Emp ID": clean_id, "Name": ename, "Late Days": len(late_log), "Late In Detail": " | ".join(late_log), "Early Out Days": len(early_log), "Early Out Detail": " | ".join(early_log)})
...
if not st.session_state.auth:
    ...
    if nav == "📊 Attendance Engine":
        ...
        if file:
            ...
            m, s, o, ex, mi = run_hr_engine(df_temp.to_json(), hols, tuple(st.session_state.corrs))
        ...
        with t1:
            with st.form("manual_emp", clear_on_submit=True):
                c1, c2 = st.columns(2)
                data = {"ID": c1.text_input("ID"), "Name": c1.text_input("Name"), "Dept": c2.text_input("Dept"), "Govt_ID_Placeholder": c1.text_input("Govt ID (Redacted)"), "Status": c2.selectbox("Status", ["Active", "Inactive"]), "Designation": c2.text_input("Designation")}
