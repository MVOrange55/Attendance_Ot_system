...
st.title("📊 Final OT System (Actual In-Time for Off/Holidays)")

def calculate_final_ot(total_hrs, is_full_ot_day):
    # Rule: Agar Off/Holiday hai toh Pura Time OT (Deduction 0)
    if is_full_ot_day:
        ot_exact = total_hrs
    else:
        # Normal din par 8.5 hours ki duty minus hogi
...
def process_data(df):
    ...
    for eid in df[emp_id_col].unique():
        ...
        # Rows identifying: Status, In Time, Out Time
        ...
        for day in date_cols:
            try:
                ...
                # Is it an OFF day or National Holiday?
                ...
                # SHIFT RULE:
                # Agar OFF/Holiday hai, toh jo Excel mein 'In' time hai wahi use karo.
                # Agar Normal din hai, toh fixed 09:30 AM use karo.
                if is_full_ot_day and in_val not in ['', 'nan', '0']:
                    t_in = parse_t(in_val)
                else:
                    t_in = time(9, 30)
...
if uploaded_file:
    ...
    if st.button("🚀 Generate Final OT List"):
        final_df = process_data(df_raw)
        if final_df is not None:
            st.success("Niyam: Off/4/21 ko Actual In-Time se Pura OT calculate kiya gaya hai.")
            st.dataframe(final_df)
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                final_df.to_excel(writer, index=False)
            st.download_button("📥 Download Final Excel", output.getvalue(), "Actual_InTime_OT_Report.xlsx")
