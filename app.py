...
if not st.session_state.auth:
    ...
    if menu == "Home / Upload":
        ...
        st.header("Step 2: Holidays")
        st.session_state.hols = st.multiselect("Select Dates:", range(1, 32), default=st.session_state.hols)

    elif menu == "Punch Correction":
        st.header("Step 3: Correction")
        with st.form("corr"):
            c_id = st.text_input("ID")
            c_date = st.number_input("Date", 1, 31)
            c_in = st.text_input("In (HH:MM)", "09:30")
            c_out = st.text_input("Out (HH:MM)", "18:00")
            if st.form_submit_button("Transfer & Update"):
                st.session_state.corrs.append({'id': c_id, 'date': c_date, 'in': c_in, 'out': c_out})
                st.success("Correction Saved!")

    elif st.session_state.data is not None:
        m, s, o, ex = run_hr_engine(st.session_state.data, st.session_state.hols, st.session_state.corrs)
        if menu == "Attendance Muster":
            st.dataframe(m.style.map(lambda v: 'background-color: #d1e7dd;' if v in ['WO','H'] else 'background-color: #f8d7da;' if v == 'AB/' else 'background-color: #fff3cd;' if v == 'P*' else ''), use_container_width=True)
        elif menu == "Summary & OT":
            st.dataframe(s, use_container_width=True)
            st.subheader("Daily OT Slab")
            st.dataframe(o, use_container_width=True)
        elif menu == "Exception Report":
            st.dataframe(ex, use_container_width=True)
        
        # Download
        buf = io.BytesIO()
        with pd.ExcelWriter(buf, engine='openpyxl') as writer:
            m.to_excel(writer, sheet_name="Muster", index=False)
            s.to_excel(writer, sheet_name="Summary", index=False)
        st.sidebar.download_button("📥 Download Report", buf.getvalue(), "Orange_Report.xlsx")
    else:
        st.markdown('<div class="report-lock-msg"><h3>🚫 Report Locked</h3><p>Pehle Excel file upload karein.</p></div>', unsafe_allow_html=True)
# --- 4. CALCULATION CORE ---
def parse_t(v):
    if pd.isna(v) or str(v).strip() in ['', 'nan', '00:00', 'None']: return None
    try:
